...
class PeirsData:
    ...
    def ensure(self, key: str, force_refresh: bool = False) -> Path:
        ...
        Para datasets con `needs_conversion=True` (p. ej. V-Dem que viene en .rda),
        descarga el archivo crudo, lo convierte a CSV y cachea ambos.
        """
    ...
    def _convert_rda_to_csv(self, key: str, rda_path: Path, csv_path: Path) -> None:
        ...
        try:
            ...
            raise ImportError(
                "pyreadr no está instalado. Correr: pip install pyreadr"
            ) from exc

        result = pyreadr.read_r(str(rda_path))
        # pyreadr devuelve un OrderedDict con el nombre del objeto R como clave.
        # Para vdem.rda la clave es 'vdem'; tomamos el primer DataFrame.
        if not result:
            raise RuntimeError(f"El archivo {rda_path} no contiene objetos legibles")
        df_name, df = next(iter(result.items()))
        logger.info("Objeto R '%s' leído: %d filas, %d columnas",
